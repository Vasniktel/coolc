...
%{
  ...
  for the location of tokens */
    ...
    */
    ...
    %type <formal> formal
    %type <formals> formals maybe_formals
    %type <case_> branch
    %type <cases> cases
    %type <expression> expr let
    %type <expressions> exprs args maybe_args
    
    /* Precedence declarations go here. */
    %right IN // IN in LET expression introduces ambiguity (not sure about 'right' though)
    %right ASSIGN
    %left NOT
    %nonassoc LE '<' '='
    %left '+' '-'
    ...
    */
    ...
    feature
    : OBJECTID '(' maybe_formals ')' ':' TYPEID '{' expr '}' ';' { $$ = method($1, $3, $6, $8); }
    ...
    | expr '@' TYPEID '.' OBJECTID '(' maybe_args ')' { $$ = static_dispatch($1, $3, $5, $7); }
    | expr '.' OBJECTID '(' maybe_args ')'            { $$ = dispatch($1, $3, $5); }
    | OBJECTID '(' maybe_args ')'                     { $$ = dispatch(object(idtable.add_string("self")), $1, $3); }
    | IF expr THEN expr FI                            { $$ = cond($2, $4, no_expr()); }
    ...
    | expr ',' args { $$ = append_Expressions(single_Expressions($1), $3); }
    | error ')'
    ;

    exprs
    : expr ';'       { $$ = single_Expressions($1); }
    | exprs expr ';' { $$ = append_Expressions($1, single_Expressions($2)); }
    | error ';'
    ;
